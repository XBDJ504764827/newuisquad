...
pub async fn login(
    ...
    let user = sqlx::query_as::<_, (i32, String, String, String, JsonValue, i32)>(
        "SELECT id, username, password_hash, role, permissions, permission_version FROM admin_users WHERE username=$1"
    ).bind(&req.username).fetch_optional(&state.pool).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;

    let (_, username, password_hash, role, permissions, permission_version) = match user {
        Some(u) => u,
