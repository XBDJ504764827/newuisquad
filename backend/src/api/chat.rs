...
pub async fn list(
    ...
    let (total,) = sqlx::query_as::<_, (i64,)>("SELECT COUNT(*) FROM chat_messages WHERE server_id=$1")
    ...
    let items = sqlx::query_as::<_, (i32, String, String, String, String, chrono::DateTime<chrono::Utc>)>(
        "SELECT server_id, player_name, steam64, message, channel, logged_at FROM chat_messages WHERE server_id=$1 ORDER BY logged_at DESC LIMIT $2 OFFSET $3"
